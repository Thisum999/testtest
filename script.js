...
function loadAttendanceFromLocalStorage() {
    ...
    if (savedData) {
        ...
        updateAttendanceLog(); // Update the display immediately
        updateStudentList(document.getElementById('classSelect').value); // Update student list checkboxes
    }
}
...
function markAttendance(admissionNumber, method = 'QR') {
    ...
    if (student) {
        ...
        saveAttendanceToLocalStorage(); // Save immediately after marking
        updateAttendanceLog();
        showGreetingModal(student.name);
...
function handleManualAttendance(admissionNumber, isChecked) {
    if (isChecked) {
        ...
        saveAttendanceToLocalStorage(); // Save immediately after removing
        updateAttendanceLog();
    }
}
...
function showScanResult(message, success) {
    ...
    resultDiv.className = 'mt-3 ' + (success ? 'success fade-in' : 'error fade-in');
...
// Generate PDF report with teacher's photo
async function generatePDFWithImage(selectedClass) {
    ...
    // Load teacher photo
    const photoUrl = teacherPhotos[selectedClass];
    if (photoUrl) {
        try {
            const imgData = await loadImageAsBase64(photoUrl);
            doc.addImage(imgData, 'JPEG', 175, 10, 25, 25); // Position: right-top corner
        } catch (error) {
...
async function saveAttendanceToServer() {
    try {
        const currentDate = new Date().toISOString().split('T')[0];
        const selectedClass = document.getElementById('classSelect').value.replace(/[A-F]/, match => '-' + match);
...
function initializeApp() {
    ...
    document.getElementById('generateClassReport').addEventListener('click', () => {
        ...
        if (selectedClass) {
            generatePDFWithImage(selectedClass); // Updated to new function
